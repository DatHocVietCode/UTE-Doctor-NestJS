...
import { COIN_DEFAULT_EXPIRE_DAYS, COIN_EXPIRY_DAYS, COIN_REWARD_RATE } from './coin-reward.config';
import { CoinSummaryBreakdownItemDto, CoinSummaryResponseDto } from './coin-summary-response.dto';
import { CoinSpendAllocation, CoinSpendAllocationDocument } from './schemas/coin-spend-allocation.schema';
...
export class CoinService {
	...
	constructor(
		...
		private readonly schedulerService: CoinExpiryReminderSchedulerService,
	) {}
	...
	async addCoins(
		...
		try {
			...
			const coinExpiresAt = expiresAt ?? new Date(Date.now() + COIN_DEFAULT_EXPIRE_DAYS * 24 * 60 * 60 * 1000);
            this.logger.debug({
                expiresAtInput: expiresAt,
				COIN_DEFAULT_EXPIRE_DAYS,
            });
			const earnTransaction = await this.recordTransaction(
				patientId,
				'earn',
				normalizedAmount,
				reason,
			...
			if (earnTransaction) {
				await this.coinExpiryReminderService.createReminderJobForEarnTransaction(earnTransaction);
			}
	...
	async rewardCoinForCompletedAppointment(
		...
		try {
			await session.withTransaction(async () => {
				...
				const rewardTransaction = await this.coinTransactionModel
					.findOne({
						appointmentId: appointmentObjectId,
						type: 'earn',
						reason: 'appointment_completed_reward',
					...
					.session(session)
					.select('_id patientId amount expiresAt')
					.exec();

				if (rewardTransaction) {
					await this.coinExpiryReminderService.createReminderJobForEarnTransaction(rewardTransaction, session);
				}
			});
