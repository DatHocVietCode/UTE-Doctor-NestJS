...
### Phase 5 completed — Reminder & expired assignment notification listeners
- **The previously-unhandled SLA events now have listeners** (closing the §3 gap). `AssignmentNotificationListener` adds `@OnEvent('appointment.assignment.reminder')` and `@OnEvent('appointment.assignment.expired')` ([../src/notification/listenners/assignment.notify.listenner.ts](../src/notification/listenners/assignment.notify.listenner.ts)). Tasks no longer fail silently at SLA boundaries.
- **New notification types + handlers:** `ASSIGNMENT_TASK_REMINDER` and `ASSIGNMENT_TASK_EXPIRED`, each persisting a `Notification` (idempotent) and bridging to the socket via `NOTIFICATION_REDIS_CHANNEL` — same pipeline as `ASSIGNMENT_TASK_CREATED` ([../src/notification/handlers/assignment-task-reminder-notification.handler.ts](../src/notification/handlers/assignment-task-reminder-notification.handler.ts), [../src/notification/handlers/assignment-task-expired-notification.handler.ts](../src/notification/handlers/assignment-task-expired-notification.handler.ts)). Registered in `NotificationService` registry + `NotificationModule`.
- **Reuses Redis online-receptionist targeting from Phase 4:** both listeners go through the shared `resolveReceptionistTargets` helper, so reminders/expiries target online receptionists for realtime and still persist for all (offline-safe) — polling queue remains the fallback.
- **Idempotency keys:** reminder = `ASSIGNMENT_TASK_REMINDER:{taskId}:{reminderCount}:{recipient}` (each reminder bump is a distinct notification, but a retry of the *same* reminder dedupes); expired = `ASSIGNMENT_TASK_EXPIRED:{taskId}:{recipient}` (expiry is a one-time transition).
- **Scheduler safety:** each listener wraps its work in try/catch and logs on failure, so a notification error can never bubble back into `AssignmentSlaScheduler` (events are fire-and-forget on EventEmitter2 anyway). No booking/SLA behavior outside notification was changed.
- **Expiry escalation:** MVP notifies receptionists; **admin escalation is documented as a future improvement** (no admin notification channel is introduced here — smallest safe change).
- **Tests:** `assignment.notify.listenner.spec.ts` adds reminder cases (per-receptionist publish + online flag, reminderCount keying for repeats-vs-retries, no-online no-throw, no-receptionist no-throw) and expired cases (stable idempotency key, no-online no-throw). Full suite: **16 suites / 146 tests passing**; `npm run build` passes. Phase 5 new/edited files lint clean except pre-existing `no-unsafe-*` notes in `notification.service.ts` (its `any`-typed normalize helpers — untouched by this work).

### Phase 6 completed — Redis atomic lock for receptionist assignment handling
- **A Redis lock now guards assignment-task processing**, so two receptionists cannot handle the same task concurrently. Added `withTaskLock(taskId, receptionistId, work)` wrapping both `acceptTask` and `assignDoctorAndSlot` ([../src/appointment/appointment-assignment-task.service.ts](../src/appointment/appointment-assignment-task.service.ts)).
- **Lock style mirrors the booking flow:** key `assignment-task:{taskId}:lock`, atomic `SET NX EX` via `RedisService.acquireLock`, TTL **30s** (short — covers one accept/assign op), value `receptionist:{accountId}` identifying the owner. Release is **compare-and-delete** (`RedisService.releaseLock`) in a `finally`, so it runs on both success and failure and never deletes another request's lock.
- **Two-layer protection (defense in depth):** the Redis lock is the first layer; the existing **DB guards remain** the second — `acceptTask`'s atomic conditional `findOneAndUpdate({status: PENDING})` and `assignDoctorAndSlot`'s ownership check + slot lock + transactional status/conflict re-checks. So even if the lock TTL lapses, double-assignment is still impossible.
- **Clear conflict, no crash:** a held lock raises `TASK_LOCK_HELD` → *"This assignment task is currently being handled by another receptionist."* via the same `throwBlocked` envelope used elsewhere (no unhandled exception).
- **Minimal-diff refactor:** the existing method bodies were extracted to `acceptTaskInternal` / `assignDoctorAndSlotInternal` (no re-indentation), with the public methods acquiring the lock and delegating — so the diff is the lock logic only, not a whole-file reformat.
- **Tests:** `appointment-assignment-task.service.spec.ts` (accept) + `appointment-assignment-task.assign.spec.ts` (assign) add: lock acquired→proceeds, lock held→`TASK_LOCK_HELD` (and DB never touched / slot lock never reached), lock released after success, lock released after a handled failure, and owner-scoped release value (cannot delete another owner's lock). The existing atomic-accept/transactional tests cover the DB second-layer guard. Full suite: **16 suites / 151 tests passing**; `npm run build` passes. (`appointment-assignment-task.service.ts` retains its pre-existing `no-unsafe-*` lint baseline — `error?.message`/`as any` in unrelated helpers; the new lock code is clean and the file was reformatted minimally to avoid diff noise.)

### Phase 7 verified — Broad booking validation & deposit flow
- **Broad booking without doctor/timeSlot is accepted by the current flow** — no code changes were needed; the behavior was verified and no blocker was found within thesis scope. `bookAppointment` branches to `bookBroadAppointment` when `broadBooking=true` *before* the doctor/slot-requiring `validateBookingRequest`, creating a `PENDING` appointment with `doctorId`/`timeSlot` omitted and `assignmentStatus = AWAITING_ASSIGNMENT`, plus a `PENDING` `AppointmentAssignmentTask` ([../src/appointment/appointment-booking.service.ts](../src/appointment/appointment-booking.service.ts)).
- **Deposit/payment is consistent and does not crash:** BHYT broad booking → `depositStatus = NOT_REQUIRED` (no payment created); DICH_VU broad booking → upfront deposit + VNPay URL; deposit-gateway failure → appointment `FAILED` + task `CANCELLED` (and `appointment.assignment.created` is **not** emitted). All covered by existing `appointment-booking.service.broad.spec.ts` tests.
- **No normal-booking regression:** the non-broad path still hard-requires doctor and timeSlot. Added two contrast regression tests (`normal booking validation (regression guard)`) asserting `Doctor is required` / `Time slot is required` for a non-broad payload missing each field.
- **Implementation rule honored:** the payment flow was **not** rewritten (no failing test / obvious bug). Verification only + 2 new tests.
- **Tests:** `appointment-booking.service.broad.spec.ts` now 10 cases. Full suite: **16 suites / 153 tests passing**.
